...
<main class="max-w-3xl mx-auto px-6 py-20">
    ...
    @if (session('error'))
        <div class="mb-8 text-sm text-red-700 bg-red-50 border border-red-200 rounded-2xl px-6 py-4">
            {{ session('error') }}
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 gap-8">
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            ...
            <div class="flex gap-6 items-start">
                ...
                <div>
                    ...
                    <p class="text-slate-500">{{ $event->date->translatedFormat('d M Y, H:i') }} • {{ $event->location }}</p>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 p-8 shadow-sm">
            <h3 class="text-xl font-bold mb-6 italic text-indigo-600 underline underline-offset-8">
                📦 Data Pemesan {{ auth()->check() ? '' : '(Tanpa Login)' }}
            </h3>

            <form method="POST" action="{{ route('checkout.store', $event) }}" class="space-y-6">
                @csrf
