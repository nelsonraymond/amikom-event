...
@section('page_subtitle', 'Daftarkan partner baru ke dalam sistem.')
...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="px-8 py-5 border-b border-slate-100">
            <h3 class="font-bold text-slate-700">Form Data Partner</h3>
            <p class="text-xs text-slate-400 mt-0.5">Isi semua field yang bertanda <span class="text-red-500">*</span></p>
        </div>

        <form action="{{ route('admin.partners.store') }}" method="POST" class="px-8 py-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <select id="logo_preset"
                    ...
                    <option value="https://placehold.co/200x200/6366f1/white?text=Partner">Indigo (Default)</option>
...
<div>
    ...
    <input type="email"
           ...
           required>
    <p class="text-xs text-slate-400 mt-1.5">Email ini dipakai partner untuk login ke dashboard mereka.</p>
    @error('email')
        <p class="text-red-500 text-xs font-medium mt-1.5">{{ $message }}</p>
    @enderror
</div>
...
<div>
    ...
    <input type="password"
           ...
           class="w-full px-4 py-3 rounded-xl border border-slate-200 text-sm font-medium text-slate-800 placeholder-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition @error('password') border-red-400 bg-red-50 @enderror"
           required>
    @error('password')
        <p class="text-red-500 text-xs font-medium mt-1.5">{{ $message }}</p>
    @enderror
            ...
            <div>
                ...
                <div class="flex items-center gap-4 p-4 bg-slate-50 rounded-xl border border-slate-100">
                    <img id="logo_preview"
                         src="https://placehold.co/200x200/e2e8f0/94a3b8?text=Preview"
                         alt="Preview"
                         class="w-16 h-16 rounded-xl object-cover border border-slate-200 shadow-sm">
                    <div>
                        <p class="text-sm font-semibold text-slate-600">Preview Logo</p>
                        <p class="text-xs text-slate-400 mt-0.5">Gambar akan muncul otomatis saat URL diisi.</p>
                    </div>
                </div>
            </div>
